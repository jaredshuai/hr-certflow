...
def test_build_certificate_coverage_report_groups_department_type_and_expiry_risk() -> None:
    ...
    safety_type = CertificateType(id=uuid.uuid4(), code="SAFETY", name="安全员证")
    electric_type = CertificateType(id=uuid.uuid4(), code="ELEC", name="电工证")
    certificates = [
        EmployeeCertificate(
    ...
    report = build_certificate_coverage_report(
        ...
        certificate_types=[safety_type, electric_type],
    ...
    assert electric_row.missing_employee_count == 2
    assert electric_row.risk_count == 3
...
def test_build_certificate_coverage_report_csv_is_excel_friendly() -> None:
    ...
    assert "证书类型,有效数,即将到期,已过期,缺失员工数,风险合计" in payload
